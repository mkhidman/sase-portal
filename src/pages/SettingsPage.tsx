  ...
  return <>
    ...
    <article className="card">
      ...
      <div className="table-wrap"><table><thead><tr><th>Admin</th><th>Kelas yang diampu</th><th>Login terakhir</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
        {filteredAdmins.length ? adminPagination.pageItems.map((admin) => <tr key={admin.id}><td><Person name={admin.name} meta={admin.email} />{admin.mustChangePassword ? <span className="badge warning">Wajib ganti password</span> : null}</td><td><div className="badge-list">{admin.assignedClassIds.length ? admin.assignedClassIds.map((id) => <span className="badge muted" key={id}>{classes.find((item) => item.id === id)?.name ?? 'Kelas'}</span>) : <span className="muted-text">Belum ada kelas</span>}</div></td><td>{admin.lastLoginAt ? formatDateTime(admin.lastLoginAt) : 'Belum pernah login'}</td><td><span className={`badge ${admin.active ? 'success' : 'danger'}`}>{admin.active ? 'Aktif' : 'Nonaktif'}</span></td><td><div className="admin-action-grid"><button className="button subtle small" disabled={!admin.active} onClick={() => openEdit(admin)}><Pencil size={14} /> Kelas</button><button className="button subtle small" disabled={!admin.active} onClick={() => openTransfer(admin)}><ArrowRightLeft size={14} /> Pindah</button><button className="button subtle small" onClick={() => openReset(admin)}><KeyRound size={14} /> Password</button><button className={`button small ${admin.active ? 'danger' : 'primary'}`} onClick={() => openStatus(admin)}>{admin.active ? <PowerOff size={14} /> : <Power size={14} />}{admin.active ? 'Nonaktifkan' : 'Aktifkan'}</button></div></td></tr>) : <tr><td colSpan={5}><div className="empty-state">Tidak ada akun Admin pada filter ini.</div></td></tr>}
      </tbody></table></div>
    ...
    <article className="card backup-card"><div className="backup-copy"><span className="backup-icon"><ShieldCheck size={20} /></span><div><h2>Backup Data Aplikasi</h2><p>Unduh snapshot JSON termasuk status, login terakhir, dan pembagian kelas Admin.</p></div></div><div className="backup-summary"><span>{jamaah.length} jamaah</span><span>{attendanceSessions.length} sesi</span><span>{admins.length} Admin</span></div><button className="button primary" onClick={downloadBackup}><Download size={16} /> Unduh Backup</button></article>
