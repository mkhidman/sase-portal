...
export function SettingsPage() {
  ...
  async function resetDemoData() {
    const approved = await confirm({
      ...
      description: 'Semua perubahan lokal pada mode demo akan dibuang dan data kembali ke keadaan awal. Data Supabase tidak terpengaruh.',
      confirmLabel: 'Reset Data Demo',
      tone: 'danger',
    })
    if (approved) resetDemo()
  }
  ...
  return <>
    ...
    <article className="card">
      ...
      <div className="table-wrap"><table role="table"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader">Admin</th><th scope="col" role="columnheader">Kelas yang diampu</th><th scope="col" role="columnheader">Login terakhir</th><th scope="col" role="columnheader">Status</th><th scope="col" role="columnheader">Aksi</th></tr></thead><tbody role="rowgroup">
        {filteredAdmins.length ? adminPagination.pageItems.map((admin) => <tr role="row" key={admin.id}><td role="cell" data-cell="primary"><Person name={admin.name} meta={admin.email} />{admin.mustChangePassword ? <span className="badge warning">Wajib ganti password</span> : null}</td><td role="cell" data-label="Kelas yang diampu"><div className="badge-list">{admin.assignedClassIds.length ? admin.assignedClassIds.map((id) => <span className="badge muted" key={id}>{classes.find((item) => item.id === id)?.name ?? 'Kelas'}</span>) : <span className="muted-text">Belum ada kelas</span>}</div></td><td role="cell" data-label="Login terakhir">{admin.lastLoginAt ? formatDateTime(admin.lastLoginAt) : 'Belum pernah login'}</td><td role="cell" data-label="Status"><span className={`badge ${admin.active ? 'success' : 'danger'}`}>{admin.active ? 'Aktif' : 'Nonaktif'}</span></td><td role="cell" data-label="Aksi" data-cell="full"><div className="admin-action-grid"><button className="button subtle small" disabled={!admin.active} onClick={() => openEdit(admin)}><Pencil size={14} /> Kelas</button><button className="button subtle small" disabled={!admin.active} onClick={() => openTransfer(admin)}><ArrowRightLeft size={14} /> Pindah</button><button className="button subtle small" onClick={() => openReset(admin)}><KeyRound size={14} /> Password</button><button className={`button small ${admin.active ? 'danger' : 'primary'}`} onClick={() => openStatus(admin)}>{admin.active ? <PowerOff size={14} /> : <Power size={14} />}{admin.active ? 'Nonaktifkan' : 'Aktifkan'}</button></div></td></tr>) : <tr role="row"><td role="cell" colSpan={5}><div className="empty-state">Tidak ada akun Admin pada filter ini.</div></td></tr>}
      </tbody></table></div>
    ...
    {isDemo ? <article className="card reset-card"><div><h2>Reset Data Demo</h2><p>Kembalikan seluruh perubahan lokal ke data awal development.</p></div><button className="button danger" onClick={() => void resetDemoData()}><RotateCcw size={16} /> Reset Demo</button></article> : null}
