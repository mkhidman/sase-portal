...
export function ClassesPage() {
  ...
  return (
    <>
      <PageHeader title="Kelas Pengajian" description="Satu jamaah dapat mengikuti lebih dari satu kelas atau jenis pengajian." actions={<Link className="button primary" to="/kenaikan-kelas"><Shuffle size={16} /> Kenaikan & Mutasi</Link>} />
        ...
        {classes.map((studyClass) => {
          ...
          return (
            <article className="card class-card" key={studyClass.id}>
              <div className="card-heading"><div><h2>{studyClass.name}</h2><p>{studyClass.name === '5 Unsur' ? 'Anggota ditentukan secara manual.' : 'Keanggotaan mengikuti data jamaah.'}</p></div><span className="badge success">Aktif</span></div>
